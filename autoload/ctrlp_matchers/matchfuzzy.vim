...
function! ctrlp_matchers#matchfuzzy#match(items, str, limit, mmode, ispath, crfile, regex) abort
    if empty(a:str)
        call clearmatches()
    ...
    let s:timer = timer_start(
                \ 10,
                \ { t -> [clearmatches(), matchadd('CtrlPMatch', ctrlp_matchers#Esc(a:str)), hlexists('CtrlPLinePre') ? matchadd('CtrlPLinePre', '^>') : '', execute('redraw')] },
                \ { 'repeat': 0 }
                \ )
    return l:items
endfunction
