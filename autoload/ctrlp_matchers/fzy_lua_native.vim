" A matching function for CtrlP using fzy-lua-native
" Arguments:
" |
...
function! ctrlp_matchers#fzy_lua_native#match(items, str, limit, mmode, ispath, crfile, regex) abort
    if empty(a:str)
        call clearmatches()
    ...
    if g:fzy_lua_native_version ==# 'native'
        " https://github.com/romgrk/fzy-lua-native/blob/master/lua/native.lua
        let l:result = v:lua.require('fzy-lua-native').filter_many(a:str, a:items)
    else
        " https://github.com/romgrk/fzy-lua-native/blob/master/lua/original.lua
        let l:result = v:lua.require('fzy-lua-native').filter(a:str, a:items)
    endif
